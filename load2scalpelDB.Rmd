...
prepareDF<-function(sp,specID){
  md<-metaData(sp)
  resDF<-data.frame(spec=specID,
                    num=md$number,
                    rt=md$retentionTime,
                    tic=totalIonCurrent(sp),
                    mz=sp@mass,
                    intensity=sp@intensity)
  return(resDF)
}
writeDT<-function(sp,specID,fname){
  md<-metaData(sp)
...
```{r prop.table}
kable(resDF, format = "latex", caption = 'Predefined parameters for load', row.names=FALSE)
```
...
```{r prepare.signal.file}
sigFname<-normalizePath('./tmp_signal.tsv')
system.time(res<-sapply(spectra,writeDT,specID=spectrumID$id,fname=sigFname))
```

```{r load.signal.file}
system(paste0('mclient  -d msinvent  -s  "COPY INTO ms.tmp_signal  FROM ',"'",sigFname,"'",';"'))
system(paste0('gzip ',sigFname))
```

```{r populate.signal.table}
...
Таблица *Peaks* содержит выделенные из сырых данных пики, для улучшения качества данных все сканы выравниваются по отношению к скану с максимальным TIC. 
```{r prepare.peaks}
imTIC<-which.max(tic)
plot(spectra[[imTIC]])
origSpectra<-spectra
...
alSpectra<-spectra
peaks2 <- detectPeaks(spectra, SNR=1, halfWindowSize=halfWindowSize,
