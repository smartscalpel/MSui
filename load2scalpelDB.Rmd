...
if(!(exists('dbname')&
     ...
     exists('pwd')&
     exists('dtPath')&
     exists('cdf.file')& #CDF file not needed any more it was processed by prepare4db, but it should be loaded into database
     exists('devID')&
     exists('solID')&
     exists('isID')&
     exists('resID')&
...
plot(spectra[[imTIC]], type="b",xlim=c(-1,1)+palmMZ,ylim=range(spectra[[imTIC]]@intensity[palmID]))
```
```{r plot.stearate}
stearMZ<-283.27
...
peakFname<-normalizePath(paste0(dirname(knitr::current_input(dir = TRUE)),'/tmp_peak.tsv.gz'))
peakFname
```
...
rs<-dbSendStatement(conn,
                        paste('INSERT INTO peak(mz,scan,intensity,norm2tic,sqrtnorm2tic,sqrtintensity,snr)',
                              ...
                              'from tmp_peak t, scan s, spectrum p ',
                              'where s.spectrumid=p.id and p.filename=t.spec and s.num=t.num);'))
...
Loaded spectra saved in `res.fname` and original CDF file saved in `cdf.fname`

```{r save.data}
save(spectra,origSpectra,ncMD,peaks,peaks0,peaks2,fpeaks2,featureMatrix2,file = res.fname)
