...
writeDTpeak<-function(id,scanDF,tic,peak,fname){
  ...
  resDF<-data.table(scan=scanID,
                    ...
                    snr=p@snr)
  resDF$scan<-as.integer(resDF$scan)
...
if (exists("scID")) {
  ...
  cat(paste("There is no empty scans for the file", cdf.fname, "\n"))
}
...
if (exists("pID")) {
  ...
  cat(paste("There is no empty peaks for the file", cdf.fname, "\n"))
}
...
specdx<-match(specT$id,scanDF$spectrumid)
if(dim(scanDF)[1]==0){
  ...
  scanDF<-dbGetQuery(conn,getScans,paste0(cdf.fname,'_%'))
}else if(any(is.na(specdx))){
  for(sidx in which(is.na(specdx))){
    cdf.fid<-paste0(cdf.fname,'_',sidx)
    spectrumID<-dbGetQuery(conn,getSpecID,cdf.fid)
    for(sp in origSpectra[intRT==sidx]){
      md<-metaData(sp)
...
```{r drop.peak.constraints, eval=FALSE}
...
peakFname
if(file.exists(peakFname)){
  file.remove(peakFname)
}
```
...
system(paste0('mclient  -d msinvent  -s  "COPY INTO ',
              ...
              '  FROM ',"'",peakFname,
              "'(scan,mz,intensity,norm2tic,sqrtnorm2tic,sqrtintensity,snr)",' ON CLIENT;"'),
       intern = TRUE)->mcOut
mcOut
...
wcn
mcOutn <- as.numeric(unlist(strsplit(trimws(mcOut), ' ')))
if (mcOutn[[1]] < wcn[[1]]){
  stop(paste('Number of affected rows is not equal to the number of rows in ', peakFname, sep = ''))
}
...
```{r add.peak.constraints, eval=FALSE}
