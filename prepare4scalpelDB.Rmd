...
for(p in unique(pr)){
  idx<-which(pr==p)
l[[p]]<-alignSpectra(spectra[idx],reference = spectra[[which.max(tic[idx])]], halfWindowSize = halfWindowSize,tolerance = tolerance)
}
al<-unlist(l)
...
# Save data
```{r save.data}
save(spectra,intRT,origSpectra,ncMD,peaks,peaks0,peaks2,fpeaks2,featureMatrix2,mdL,mdLspectra,solID,isID,smplID,stID,stpID,exData,cdf.file,cdf.fname,file = 'spectra.RData')
```
