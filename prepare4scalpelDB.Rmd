...
writeDTpeak<-function(id,specID,tic,peak,fname,intRT){
  p<-peak[[id]]
  md<-metaData(p)
  if(length(p)>0){
  #cat('Writing scan',md$number,md$retentionTime,'\n')
  resDF<-data.table(spec=paste0(specID,'_',intRT[id]),
                    num=md$number,
                    mz=p@mass,
...
spectra<-import(cdf.file,verbose=TRUE,removeEmptySpectra=FALSE)
...
qth<-min(absTicThreshold,quantile(tic,probs = ticThreshold))
#plot(xraw@tic,tic)
plot(tic)
abline(h = qth)
spectra <-spectra[tic>qth]
tic<-sapply(spectra,totalIonCurrent)
rt<-sapply(spectra,function(.x)metaData(.x)$retentionTime)
num<-sapply(spectra,function(.x)metaData(.x)$number)
intRT<-findInterval(rt,protocol$rt)
mdL<-lapply(1:length(intRT),function(.x)list(number=num[.x],Polarity=as.character(protocol$Polarity[intRT[.x]]),
                                   Detector=as.character(protocol$Detector[intRT[.x]]),
                                   Range=as.character(protocol$Range[intRT[.x]]),
                                   protocol=paste(protocol$Polarity[intRT[.x]],
                                                  protocol$Detector[intRT[.x]],
                                                  protocol$Range[intRT[.x]],sep = '.')))
mnum<-sapply(mdL,function(.x).x$number)
tic<-sapply(spectra,totalIonCurrent)
...
```{r save.sig}
save(spectra,intRT,ncMD,mdL,solID,isID,smplID,stID,stpID,cdf.file,cdf.fname,exData,protocol,file = 'spectra.RData')
...
for(p in unique(pr)){
  idx<-which(pr==p)
  k<-length(l)+1
l[[k]]<-alignSpectra(spectra[idx],reference = spectra[[which.max(tic[idx])]], halfWindowSize = halfWindowSize,tolerance = tolerance)
names(l)[k]<-p
}
al<-unlist(l)
snum<-sapply(al,function(.x)metaData(.x)$number)
spectra<-al[order(snum)]
...
pnum<-sapply(peaks2,function(.x)metaData(.x)$number)
idx<-match(pnum,mnum)
mdLspectra<-mdL
mdL<-mdL[idx]
ticSpectra<-tic
tic2<-tic[idx]
pr<-sapply(mdL,function(.x).x$protocol)
fpeaks2 <- filterPeaks(peaks2, minNumber = 10,labels=pr,mergeWhitelists=FALSE)
featureMatrix2 <- intensityMatrix(fpeaks2, spectra)

```

```{r prepare.peak.file,eval=FALSE}
peakFname<-normalizePath(paste0(dirname(knitr::current_input(dir = TRUE)),'/tmp_peak.tsv'))
system.time(res<-sapply(c(1:length(fpeaks2)),writeDTpeak,specID=cdf.fname,tic=tic,peak=fpeaks2,fname=peakFname,intRT=intRT))
```

# Save data
```{r save.data}
save(spectra,intRT,origSpectra,ncMD,peaks,peaks0,peaks2,fpeaks2,featureMatrix2,tic2,mdL,mdLspectra,solID,isID,smplID,stID,stpID,exData,cdf.file,cdf.fname,file = 'spectra.RData')
```
