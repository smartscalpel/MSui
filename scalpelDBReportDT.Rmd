...
ni.mode<-'select id,mode,dt,filename  from spectrum where s.dt>? and mode is null'
ni.exptype<-'select id,exptype,dt,filename  from spectrum where s.dt>? and exptype is null'
```
...
Report is prepared for all spectra measured after `r date`.
...
## Counts
```{r diag, cache=FALSE}
dCnt<-dbGetQuery(conn,diagnosisQ,date)
...
```{r exT, cache=FALSE}
etCnt<-dbGetQuery(conn,expTypeQ,date)
panderCnt(etCnt)
```

```{r is, cache=FALSE}
isCnt<-dbGetQuery(conn,ionSourceQ,date)
panderCnt(isCnt)
```

```{r res, cache=FALSE}
resCnt<-dbGetQuery(conn,resQ,date)
panderCnt(resCnt)
```


```{r specT, cache=FALSE}
spec<-dbGetQuery(conn,specTotQ,date)
...
## Empty spectra
```{r empty.spectra}
resNI<-dbGetQuery(conn,emptyScansQ,date)
panderCnt(resNI)
```

## Empty scans
```{r empty.scans}
resNI<-dbGetQuery(conn,emptyPeaksQ,date)
