...
for(f in fl){
  cat(f,'\n')
  if(!fie.exists(paste0('/var/workspaceR/scalpelData/annot/LipidMaps/',f,'/image.RData'))){
    cat('loading',f,'\n')
load(paste0(dtPath,f))
  ...
  cat('datA',dim(datA),'\n')
  system(paste0('mkdir -p /var/workspaceR/scalpelData/annot/LipidMaps/',f,'/'))
  resLipidMaps<-multilevelannotation(datA,num_nodes = 5,
                            outloc = paste0('/var/workspaceR/scalpelData/annot/LipidMaps/',f),
  ...
  save.image(paste0('/var/workspaceR/scalpelData/annot/LipidMaps/',f,'/image.RData'))
