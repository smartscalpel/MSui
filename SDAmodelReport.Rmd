...
df<-cbind(data.frame(diag=cls),as.data.frame(predict.fm))
...
sdf<-cbind(data.frame(diag=cls),as.data.frame(sfm))
...
combine_res<-as.data.frame(
  matrix(0,ncol=length(scheme),
         ...
         dimnames = list(c('train','test'),
                         scheme)))
train_err<-sapply(train.best,getRMSD,obs=cls)
...
for (i in scheme) {
  combine_f[[i]] <- Forecast_comb(obs = (as.numeric(cls)-1),
                                  fhat = train.best,
                                  fhat_new=test.best,
                                  Averaging_scheme = i)
