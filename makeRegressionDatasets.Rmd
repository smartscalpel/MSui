...
########### metadata queries #############
tis.diag<-'select id,label,diagnosis from tissue where label=?'
...
#host<-'127.0.0.1'
host<-'192.168.5.133'
fname<-'/Volumes/AS_Elements/Scalpel/DBData/peaks/RegressionMetaData.tsv'
logname<-'/Volumes/AS_Elements/Scalpel/DBData/peaks/RegressionMetaData.log'
```
...
jdx<-which(!is.na(idx))
tbl$diagnosis[jdx]<-tis.diagDF$diagnosis[idx[jdx]]
...
```{r save.spectrum}
rdatfile<-sub('tsv$','RData',fname)
rdsfile<-sub('tsv$','rds',fname)
tsvfile<-sub('tsv$','spec.tsv',fname)
