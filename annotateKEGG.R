...
for(f in fl){
  cat(f,'\n')
  if(!fie.exists(paste0('/var/workspaceR/scalpelData/annot/KEGG/',f,'/image.RData'))){
    cat('loading',f,'\n')
load(paste0(dtPath,f))
  ...
  cat('datA',dim(datA),'\n')
  system(paste0('mkdir -p /var/workspaceR/scalpelData/annot/KEGG/',f,'/'))
  resKEGG<-multilevelannotation(datA,num_nodes = 5,
                            outloc = paste0('/var/workspaceR/scalpelData/annot/KEGG/',f),
  ...
  save.image(paste0('/var/workspaceR/scalpelData/annot/KEGG/',f,'/image.RData'))
